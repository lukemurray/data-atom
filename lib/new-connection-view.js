...
class NewConnectionView {
  constructor(onConnect) {
    ...
    this.url.setAttribute('placeholder-text', this.urlProtocol + this.placeholderUrlPart);
  ...
  updateDbType(e) {
    for (var n in this.dbType.children()) {
      if (n.selected) {
        this.urlProtocol = $(n).attr('value');
        this.dbPort.setPlaceholderText($(n).attr('data-port'));
        this.url.setPlaceholderText(this.urlProtocol + this.placeholderUrlPart);
        return;
      }
    }
  }
