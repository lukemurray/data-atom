...
class NewConnectionView {
  ...
  buildUrl() {
    ...
    var userPass = this.dbUser.getModel().getText() + ':' + this.dbPassword.getModel().getText();
    if (userPass != ':')
       urlStr += userPass + '@';
    ...
    urlStr += '/' + this.dbName.getModel().getText();
    if (this.dbOptions.getModel().getText())
       urlStr += '?';
       _.each(this.dbOptions.getModel().getText().split(','), (s) => urlStr += _s.trim(s) + '&');
