...
class SQLMetaProvider {
  ...
  getTableNames(editor, tableSearch, schema) {
    ...
    const options = {
      ...
      keys: [
        ...
        { name: 'schemaName', weight: 0.5 }
      ]
    };
    const searcher = new Fuse(tables, options);
    var results = searcher.search(tableSearch);
  ...
  getColumnNames(editor, columnSearch, objectName) {
    ...
    const searcher = new Fuse(columns, options);
    var results = searcher.search(columnSearch);
    let valid = results.filter((col) => {
      if (objectName)
