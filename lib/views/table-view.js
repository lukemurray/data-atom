...
export default class DataResultView {
    ...
    return  <table ref='table' tabIndex='-1'>
              ...
              {this.state.rows.map((row, cnt) =>
                <tr>
                  ...
                  {row.map((data, col) =>
                    <td className='result-cell cell-width'>{typeof data === 'object' ? JSON.stringify(data) : data.toString()}</td>
                  )}
                </tr>
              )}
              </tbody>
  ...
  columnResizeStart(event) {
    [].slice.call(this.refs.table.getElementsByTagName('td')).forEach(c => { c.style.maxWidth = c.clientWidth; c.classList.remove('cell-width'); });
