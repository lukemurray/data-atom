...
class NewConnectionDialog {
          ...
          <div className='row-item-flex'>
            <select className='form-control' tabindex='0' ref='connections'>
              ...
              {this.loadedConnections.map(con =>
                <option value={con.getUrl()}>{con.name}</option>
              )}
            </select>
          </div>
  ...
  separateUrl() {
    ...
    this.refs.dbName.getModel().setText(Utils.nullToEmpty(dbConfig.database));
    ...
    var dbConfig = new DbConnectionConfig(
      {
        ...
        'server' : this.refs.dbServer.getModel().getText(), // note that this is server\instance in general
  ...
  connectAndSave() {
    ...
    DbFactory.saveConnection(connection).then(() => {
      ...
      this.onConnectClicked(this.buildUrl(), connection.name);
      this.close();
    });
  }
  ...
  connect() {
    ...
    if (this.onConnectClicked && url !== '') {
      this.onConnectClicked(url, this.refs.connectionName.getModel().getText());
      this.close();
    }
  }
}
