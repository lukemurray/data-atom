...
class NewConnectionDialog {
  ...
  buildUrl() {
    ...
    if (dbOptions) {
      urlStr += '?';
      dbOptions.split(',').map(s => urlStr += _s.trim(s) + '&');
      dbOptions.split(',').map(s => this.state.escapedUrl += encodeURIComponent(_s.trim(s)) + '&');
      urlStr = _s.rtrim(urlStr, '&');
      this.state.escapedUrl = _s.rtrim(this.state.escapedUrl, '&');
    }
