...
export default class DataDetailsView {
  ...
  buildParts(element, database) {
    ...
    for(var i = 0; i < parts.length; i++) {
      ...
      li.className = parts[i].toLowerCase() + ' collapsed list-nested-item';
      ...
      if(target.children.length < 2) {
        switch(true) {
          ...
          case target.classList.contains('tables'):
            this.buildTableList(target, target.parentElement.parentElement.children[0].innerText);
            break;
          case target.classList.contains('tbl'):
            this.buildTableDetails(target, target.children[0].innerText, target.parentElement.parentElement.parentElement.parentElement.children[0].innerText);
            break;
          default:
            break;
        }
