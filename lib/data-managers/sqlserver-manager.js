...
export default class SqlServerManager extends DataManager {
  ...
  getTableDetails(database, tables) {
    ...
    return this.execute(database, 'select column_name, data_type, character_maximum_length, table_schema from INFORMATION_SCHEMA.COLUMNS where table_name IN ' + sqlTables + ' order by table_name, ordinal_position;',
      results => {
        var columns = [];
        for(var i = 0; i < results[0].rows.length; i++) {
          columns.push({
