...
class DbFactory {
  getSupportedDatabases() {
    return [
      ...
      {name: 'Metabase', prefix: 'metabase', port: 443}
    ];
  }
  ...
  createDataManagerForUrl(url, connectionName) {
    dbConfig = new DbConnectionConfig(url);

    switch (dbConfig.protocol) {
      case 'mysql': {
        var MysqlManager = require('./mysql-manager');
        return new MysqlManager(dbConfig, connectionName);
      }
      case 'postgres':
      case 'postgresql': {
        var PostgresManager = require('./postgres-manager');
        return new PostgresManager(dbConfig, connectionName);
      }
      case 'mssql': {
        var SqlServerManager = require('./sqlserver-manager');
        return new SqlServerManager(dbConfig, connectionName);
      }
      case 'metabase': {
        var MetabaseServerManager = require('./metabase-manager');
        return new MetabaseServerManager(dbConfig);
      }
      default:
        return null;
    }
