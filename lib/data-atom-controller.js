...
class DataAtomController {
  constructor(serializeState) {
    ...
    this.mainView.on('data-atom:new-connection', () => this.createNewConnection());
    this.mainView.on('data-atom:disconnect', () => this.onDisconnect());
    this.mainView.on('data-atom:connection-changed', () => this.onConnectionChanged());
    this.mainView.on('data-atom:result-view-height-changed', (e) => this.currentViewState.height = $(e.target).height());
    ...
    atom.workspaceView.on('pane-container:active-pane-item-changed', () => this.onActivePaneChanged());
  }
  // show the results view for the selected editor
  onActivePaneChanged() {
    this.mainView.hide();
    this.currentViewState = null;
    if (atom.workspace.getActiveEditor() && atom.workspace.getActiveEditor().id) {
      this.currentViewState = this.viewToEditor[atom.workspace.getActiveEditor().id];
  ...
  getOrCreateCurrentResultView() {
    if (!this.currentViewState && !this.viewToEditor[atom.workspace.getActiveEditor().id]) {
      this.viewToEditor[atom.workspace.getActiveEditor().id] = {view: new DataResultView(), isShowing: false, dataManager: null, height: 200};
    }
  ...
  createNewConnection(thenDo) {
    ...
    var ncv = new NewConnectionView( (url) => {
