...
import fs from 'fs';
import DataAtomView from './data-atom-view';
import DataResultView from './data-result-view';
import DataDetailsView from './data-details-view';
...
class DataAtomController {
  ...
  toggleView() {
    ...
    if (this.mainView.isShowing)
      ...
      this.showResultsView();
    this.getOrCreateCurrentResultView().isMainViewShowing = this.mainView.isShowing;
  }
    ...
    if (!this.newConnectionDialog) {
      this.newConnectionDialog = new NewConnectionDialog( (url) => {
        ...
        if (this.connectionList[dbManager.getConnectionName()]) {
          // duplicate connection, just select the connection
