...
var NewQueryDialog = require('./new-query-dialog');
...
class DataAtomController {
  constructor(serializeState, statusBarManager) {
    ...
    atom.commands.add('atom-workspace', 'data-atom:execute-query', () => this.executeQuery());
  ...
  /**
   * End of save connections feature
   */

  getQuery() {
    var editor = atom.workspace.getActiveTextEditor();
    if(!this.mainView.headerView.fileExecuted.isShowing)
      this.mainView.headerView.fileExecuted.show();
    if(editor.getSelectedText()) {
      this.mainView.headerView.fileExecuted.text('Results of selected text');
      return editor.getSelectedText();
    } else if((/p?sql$/).test(editor.getTitle())) {
      this.mainView.headerView.fileExecuted.text('Results of file execution');
      return editor.getText();
    } else if(this.mainView.isShowing) {
      this.mainView.headerView.fileExecuted.text('Results of query');
      return this.mainView.headerView.getQuery();
    } else {
      this.mainView.headerView.fileExecuted.text('Results of query');
      return '';
    }
  ...
  newQuery(thenDo) {
    if (!this.newQueryDialog)
      this.newQueryDialog = new NewQueryDialog( (query) => {
        this.mainView.headerView.setQuery(query);
        if(thenDo)
          thenDo(query);
        else
          this.execute();
      });
    this.newQueryDialog.show();
  }
  ...
  executeQuery() {
    ...
    if(!this.mainView.headerView.fileExecuted.isShowing)
      this.mainView.headerView.fileExecuted.show();
    this.mainView.headerView.fileExecuted.text('Results of query');
    this.actuallyExecute(currentViewState, this.mainView.headerView.getQuery());
  }
  ...
  execute() {
    ...
    if (!currentViewState || !currentViewState.dataManager)
      this.createNewConnection(() => {
        if (query === '')
              this.newQuery((q) => this.actuallyExecute(currentViewState, q));
        else
