...
export default class DataAtomController {
  constructor(serializeState, statusBarManager) {
    ...
    atom.commands.add('atom-workspace', 'data-atom:refresh', (event) => this.refresh(event));
    atom.commands.add('atom-workspace', 'data-atom:create-script', (event) => this.createScript(event));
    atom.commands.add('atom-workspace', 'data-atom:select-script', (event) => this.selectScript(event));
    atom.commands.add('atom-workspace', 'data-atom:insert-script', (event) => this.insertScript(event));
    atom.commands.add('atom-workspace', 'data-atom:update-script', (event) => this.updateScript(event));
    atom.commands.add('atom-workspace', 'data-atom:delete-script', (event) => this.deleteScript(event));
  ...
  setupAutocomplete(database, dbManager) {
    ...
    if (dbManager) {
      ...
      dbManager.getTableNames(database, 'BASE TABLES', (tables) => {
  ...
  refresh(event) {
    //TODO: determine object to refresh based on element that called the event
    switch(event.target.className) {
      case 'db-name':
        var obj = event.target.parentElement;
        if(!!obj.children[1]) {
          obj.removeChild(obj.children[1]);
          this.detailsView.buildParts(obj, obj.children[0].innerText)
        }
        break;
      case 'part-name':
        var obj = event.target.parentElement;
        if(!!obj.children[1]) {
          ...
          switch (event.target.innerText) {
            ...
            case 'Sequences':
              ...
              break;
            case 'Tables':
              this.detailsView.buildTableList(obj, this.detailsView.getDatabaseName(obj));
              break;
            case 'Views':
              //TODO
              break;
            default:
              console.log('Unhandled part-name -> ' + event.target.innerText);
              break;
          }
        }
        break;
      case 'table-name':
        ...
        if(!!obj.children[1]) {
          obj.removeChild(obj.children[1]);
          this.detailsView.buildTableDetails(obj, obj.children[0].innerText, this.detailsView.getDatabaseName(obj));
        }
        break;
      default:
        console.log('Unhandled target -> ' + event.target);
    }
  }
  ...
  createScript(event) {
    switch (event.target.className) {
      case 'db-name':
        // TODO: if using query box then add stmt there otherwise open a new tab?
        break;
      case 'table-name':
      var currentViewState = this.getOrCreateCurrentResultView();
      var dbManager = this.getDataManager(currentViewState.connectionName);
      dbManager.getCreateStatement(this.detailsView.getDatabaseName(event.target.parentElement), event.target.innerText, stmt => {
        if(currentViewState.useEditorAsQuery) {

        } else {
          this.mainView.queryEditor.getModel().setText(stmt);
  ...
  selectScript(event) {
    switch (event.target.className) {
      ...
      case 'view-name':
        var currentViewState = this.getOrCreateCurrentResultView();
        var dbManager = this.getDataManager(currentViewState.connectionName);
        dbManager.getSelectStatement(this.detailsView.getDatabaseName(event.target.parentElement), event.target.innerText, stmt => {
          if(currentViewState.useEditorAsQuery) {
  ...
  insertScript(event) {
    switch (event.target.className) {
      case 'table-name':
        var currentViewState = this.getOrCreateCurrentResultView();
        var dbManager = this.getDataManager(currentViewState.connectionName);
        dbManager.getInsertStatement(this.detailsView.getDatabaseName(event.target.parentElement), event.target.innerText, stmt => {
          if(currentViewState.useEditorAsQuery) {
  ...
  updateScript(event) {
    switch (event.target.className) {
      case 'table-name':
        var currentViewState = this.getOrCreateCurrentResultView();
        var dbManager = this.getDataManager(currentViewState.connectionName);
        dbManager.getUpdateStatement(this.detailsView.getDatabaseName(event.target.parentElement), event.target.innerText, stmt => {
          if(currentViewState.useEditorAsQuery) {
    ...
    switch (event.target.className) {
      case 'table-name':
        if(currentViewState.useEditorAsQuery) {
          ...
          this.mainView.queryEditor.getModel().setText('DELETE FROM ' + event.target.innerText + ' WHERE <condition>;');
          // make sure it's showing the results view
          this.showMainView();
        }
        break;
